...
class Reranker(RAGStep):
    ...
    def generate(self, query: Query, chunks: list[EmbeddedChunk] | list[str], keep_top_k: int) -> list[EmbeddedChunk]:
        """
        mock: set to True to test the cross encoder on strings instead of chunks
        """
        if self._mock:
            logging.info("Reranker: Running in mock")
            assert isinstance(chunks[0], str), "str not provided for Reranker chunk in mock mode"
            query_doc_tuples = [(query.content, content) for content in chunks]
        else:
            assert isinstance(chunks[0], EmbeddedChunk), "list[EmbeddedChunk] not provided for Reranker chunk"
