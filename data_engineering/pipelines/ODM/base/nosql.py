...
class NoSQLBaseDocument(BaseModel, Generic[T], ABC):
    ...
    # override equality operation to compared ids
    ...
    def from_mongo(cls: Type[T], data: dict) -> T:
        """Convert '_id' (string) to 'id' (UUID object)"""
        if not data:
    ...
    def to_mongo(self: T, **kwargs) -> dict:
        """Convert 'id' (UUID object) to '_id' (string)"""
    ...
    def model_dump(self: T, **kwargs):
        # Pydantic objects have a default model_dump method that can convert
        # Pyndatic instance to dicts
    ...
    def save(self: T, **kwargs) -> T | None:
        """saves object instance to mongodb collection"""
        collection = _database[self.get_collection_name()]
        try:
            mongo_dict = self.to_mongo(**kwargs)
            collection.insert_one(mongo_dict)
            return self
        except errors.WriteError:
            logging.info("failed to insert document to mongo")
            return None
    ...
    def get_or_create(cls: Type[T], **filter_options) -> T:
        ...
        try:
            ...
            logging.exception(
                "Failed to retrieve document with filter options: {filter_options}"
            )
    ...
    def bulk_insert(cls: Type[T], documents: list[T], **kwargs) -> bool:
        ...
        try:
            ...
            logging.error(f"Failed to insert documents of type {cls.__name__}")
            return False
